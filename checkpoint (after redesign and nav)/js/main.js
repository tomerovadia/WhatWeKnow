...
$(document).on('click', ".question",
	function(e){
		...
		if(answers.is(":visible")){
			...
			question.parent().parent().find(".desktop-question-controls").css("background-color","lightgray").css("border-right","2px solid black") //Change color of the control panel (Desktop)
		// Otherwise, slide the answer panel down and watch the scroll so the question can be fixed if it reaches top
			...
			question.parent().parent().find(".desktop-question-controls").css("background-color","#F5F5F5").css("border-right","none") //Change color of the control panel (Desktop)
		}
	}
)
})
